...
import { Booking, Contractor, Payment } from "@/types/prismaTypes";
      ...
      <div className="flex gap-5">
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <h2 className="text-2xl font-bold my-2">{contractor.name}</h2>
            <div className="flex items-center mb-2">
              <MapPin className="w-5 h-5 mr-1" />
              <span>
                {contractor.location.city}, {contractor.location.country}
              </span>
            </div>
          </div>
...
const ServiceRecord = () => {
  ...
  } = useGetContractorQuery(Number(id));
  ...
  if (contractorLoading || bookingsLoading || paymentsLoading)
    return <Loading />;
  if (!contractor || contractorError)
    return (
  ...
  const currentBooking = bookings?.find(
    (booking) => booking.contractorId === contractor.id
  );
